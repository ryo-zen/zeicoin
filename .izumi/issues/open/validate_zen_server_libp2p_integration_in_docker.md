...
updated_at: 2026-03-28T14:12:31+00:00
---
...
- [ ] A Docker-based `zen_server` topology starts with libp2p-era bootstrap config using multiaddrs, not legacy `host:port`
- [ ] At least two `zen_server` containers complete the libp2p connection path successfully and reach steady-state sync without crashing
- [ ] Restarting a bootstrap node causes at least one peer to redial and reconnect successfully
- [ ] Test logs show the libp2p path is being used end-to-end and do not require `209.38.84.23` or any other legacy pre-libp2p node
- [ ] The exact Docker command sequence and expected pass/fail signals are documented for reruns
...
Run this before ZEI-54. Docker is the cheapest next rung after loopback because it exercises separate network namespaces and restart behavior, but it still does not replace the final real-host validation step.
