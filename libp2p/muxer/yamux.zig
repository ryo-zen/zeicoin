...
test "yamux rst closes only target stream" {
    ...
    const ResponderCtx = struct {
        ...
        fn run(ctx: *@This()) anyerror!void {
            ...
            try session.writeFrame(.data, FLAG_RST, stream_1.core.stream_id, 0, "");
...
test "yamux inbound accept backlog limit is enforced at 64 streams" {
    ...
    const ResponderCtx = struct {
        ...
        fn run(ctx: *@This()) anyerror!void {
            ...
            mutexLock(&session.pending_accept_mu);
            const queued = session.pending_accept.items.len;
            mutexUnlock(&session.pending_accept_mu);
            if (queued != MAX_PENDING_ACCEPT) return error.TestExpectedEqual;
    ...
    var futures: [MAX_PENDING_ACCEPT + 1]OpenStreamFuture = undefined;
    for (&futures) |*future| {
